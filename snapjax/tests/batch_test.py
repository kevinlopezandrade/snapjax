...
def test_batch_no_sp_no_scan():
    T = 50
    B = 8
    model = get_stacked_rnn(4, 20, 20, sparse=False)
...
def test_batch_sp_no_scan():
    T = 50
    B = 8
    model = get_stacked_rnn(4, 20, 20, sparse=True)
    batch_inp = get_random_batch(B, T, model)
    batch_out = get_random_batch(B, T, model)
...
def test_batch_sp_scan():
    ...
    mapped_rtrl = jax.vmap(jtu.Partial(rtrl, model, use_snap_1=True, use_scan=True))
    acc_loss_batch, acc_grads_batch, _ = mapped_rtrl(batch_inp, batch_out)
    ...
    for i in range(B):
        ...
        acc_loss, acc_grads, _ = rtrl(model, inp, out, use_snap_1=True, use_scan=True)
