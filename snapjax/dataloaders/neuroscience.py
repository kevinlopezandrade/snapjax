...
def flipflop(
    ...
    assert (
        dim_out == dim_in
    ), "Output and input dimensions must agree:    dim_out != dim_in."
    assert np.max(choices) <= (
        dim_in - 1
    ), "The max choice must agree with input dimension!"
    ...
    def task(batch_size):
        ...
        for b_idx in range(batch_size):
            ...
            test_intervals = np.array(
                [16.55, 9.35, 14.80, 11.73, 12.17, 6.50, 13.06, 19.08, 13.19]
            )
            ...
            while True:
                ...
                if test and b_idx == 0 and i_interval < len(test_choices):
                    ...
                    interval = np.random.uniform(
                        stim_delay_duration_min, stim_delay_duration_max
                    )
                ...
                # Target
                target_samp[
                    idx_t + decision_delay_duration_discrete : idx_tp1, choice
                ] = sign
                # Mask
...
def mante(
    ...
    if coherences is None:
        ...
        coherences = np.array([-8, -4, -2, -1, 1, 2, 4, 8]) / 8.0
    ...
    def task(batch_size):
        ...
        for b_idx in range(batch_size):
            ...
            input_sens_samp = (
                np.random.randn(n_t_max, dim_sens) * rel_input_std / np.sqrt(dt)
            )
            ...
            if b_idx < (1 - fraction_catch_trails) * batch_size:
                ...
                input_cont_samp[stim_begin:stim_end, context] = 1.0
...
def input_driven_fp(
    ...
    def task(batch_size):
        ...
        mask_batch[:, -decision_duration_discrete:] = 1.0
